...
local function getTwoClick(skill)
	local Skill = _G[skill]

	if Board:IsTipImage() then
		return false
	else
		return Skill.TwoClick or false
	end
end
...
local function getSecondTargetArea(skill, p1, p2, ...)
	local Skill = _G[skill]

	if Board:IsTipImage() then
		return Skill_Repair_Orig:GetSecondTargetArea(p1, p2, ...)
	end

	return Skill:GetSecondTargetArea(p1, p2, ...)
end
...
local function getSkillEffect(skill, p1, p2, _, ...)
	...
	if Board:IsTipImage() then
		...
		p2 = t.Target
		if Skill.TwoClick then
			local p3 = t.Second_Click
			return Skill:GetFinalEffect(p1, p2, p3, Skill, ...)
		end
	end
...
local function getFinalEffect(skill, p1, p2, p3, _, ...)
	local Skill = _G[skill]
...
local function getField(skill, field, ...)
	...
	elseif field == "SecondTargetArea" then
		return getSecondTargetArea(skill, ...)
	elseif field == "FinalEffect" then
		return getFinalEffect(skill, ...)
	elseif field == "TwoClick" then
		return getTwoClick(skill)
	end
...
local function overrideRepairSkill()
	function Skill_Repair:GetSecondTargetArea(p1 ,p2)
		local ret = PointList()
		ret:push_back(p1)
		return ret
	end
	function Skill_Repair:GetFinalEffect(p1 ,p2, p3)
